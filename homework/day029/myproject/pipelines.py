...
class JSONPipeline(object):
    ...
    def close_spider(self, spider):
        ...
        # 假如 PTT 爬蟲有給定存檔檔名，就使用給予的檔名 --day027
        #if spider.name == 'PPTCrawler' and spider.filename and spider.board:
        if spider.name == 'PPTCrawler' and spider.filename:
		#if spider.name == 'PPTCrawler' and spider.filename:
            if Path(spider.filename).suffix == '.json':
                self.store_file_path = self.dir_path / '{board}-{filename}'.format(board=spider.board,filename=spider.filename)
            else:
                self.store_file_path = self.dir_path / '{board}-{filename}.json'.format(board=spider.board,filename=spider.filename)
